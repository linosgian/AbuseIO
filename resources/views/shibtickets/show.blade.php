		...
		<div class="col-md-8 col-md-offset-4 text-right">
			<div class="btn-group" role="group" aria-label="...">
				...
				<ul class="dropdown-menu dropdown-menu-right">
					...
					<li{!! ($ticket->status_id == 'OPEN') ? ' class="disabled"' : '' !!}>{!! link_to_route('shib.tickets.status', trans('tickets.open'), [$ticket->id, 'open']) !!}</li>
					<li{!! ($ticket->status_id == 'CLOSED') ? ' class="disabled"' : '' !!}>{!! link_to_route('shib.tickets.status', trans('tickets.closed'), [$ticket->id, 'closed']) !!}</li>
					<li role="separator" class="divider"></li>
					<li{!! ($ticket->status_id == 'ESCALATED') ? ' class="disabled"' : '' !!}>{!! link_to_route('shib.tickets.status', trans('tickets.escalated'), [$ticket->id, 'escalated']) !!}</li>
					<li{!! ($ticket->status_id == 'IGNORED') ? ' class="disabled"' : '' !!}>{!! link_to_route('shib.tickets.status', trans('tickets.ignored'), [$ticket->id, 'ignored']) !!}</li>
					<li{!! ($ticket->status_id == 'RESOLVED') ? ' class="disabled"' : '' !!}>{!! link_to_route('shib.tickets.status', trans('tickets.resolved'), [$ticket->id, 'resolved']) !!}</li>
				</ul>
			</div>
		</div>
	</div>
